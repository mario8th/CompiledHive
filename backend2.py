...
class BackendData:
    ...
    def checkObstacleCollide(self):
        ...
        for obstacle in self.obstDict:
            for droneCount, drone in enumerate(self.currentLocations):
                if(pointWithinRectangle(drone, self.obstDict[obstacle][0], self.obstDict[obstacle][1])):
                    logString = "Drone " + self.fullDroneList[droneCount] + " at " + str(drone) + ' in obstacle ' + str(obstacle) + '\n'
                    self.toLog('E', logString)
    ...
    def getFunctionDests(self):
        ...
        for path in self.pathDict:
            ...
            try:
                ...
                logString = "The file " + path + ".py contains errors\n"
                print(logString)
                print(str(type(ex).__name__))
                print(str(ex.args))
                self.toLog('E', logString)
                newDests = paramLocations
...
def receivedLocations(data):
    ...
    logString = ""
    for droneCount, drone in enumerate(backend.fullDroneList):
        logString += "Drone " + drone + ': ' + str(backend.currentLocations[droneCount]) + '\n'
    backend.toLog('L', logString)
